...
function Get-IIsConfiguration {
    ...
    Begin {
       ...
       # $outputFile = "/IISConfiguration_" + (get-date -f MM_dd_yyyy_HH_mm_ss).ToString() + ".csv"
        ...
        if (!$ShowIISFeature) {
            if (($ShowIISFeature -eq 0)) {
                $ShowIISFeature = $false
            } else {
                $ShowIISFeature = $true
            }
        }
    ...
    Process {   
        ...
        try {
            if ($ShowWindowsVersion) {
                ...
                $ourObject | Add-Member -MemberType NoteProperty -Name "IIS HostName" -Value $server_name
            }
            ...
            if ($ShowIISFeature) {
                # Need to execute below command if IIS doesn't work 
                ...
                foreach ($item in $IisSites) {
                    ...
                    $index = $index + 1
                    $propKey = "Websites " + $index
                    $ourObject | Add-Member -MemberType NoteProperty -Name $propKey -Value $websiteName
                }
                $output += "`n ==========================================="
            ...
            if ($showSSLCertificate) {
                $sslcert = Get-ChildItem -Path "Cert:\LocalMachine\My"  | Select Subject
                $index = 0
                foreach ($item in $sslcert) {
                    $index = $index + 1
                    $certKey = "Certificate " + $index
                    $ourObject | Add-Member -MemberType NoteProperty -Name $certKey -Value $item.Subject
                } 
                $sslcert2 = Get-ChildItem -Path "Cert:\CurrentUser\My"  | Select Subject
                foreach ($item2 in $sslcert2) {
                    $index = $index + 1
                    $certKey2 = "Certificate " + $index
                    $ourObject | Add-Member -MemberType NoteProperty -Name $certKey2 -Value $item2.Subject
                } 
            }
            ...
            if ($ShowCompressionSettingImageStaticTypes) {
                if (((Get-WebConfigurationProperty -pspath 'MACHINE/WEBROOT/APPHOST' -filter "system.webServer/httpCompression" -Name "staticTypes").Collection  | Where-Object { $_.mimeType -eq 'image/svg+xml' }).Length -eq 1) {
                    ...
                    $ourObject | Add-Member -MemberType NoteProperty -Name "Compression Setting SVGXML StaticType" -Value "Value - image/svg+xml configured in IIS Configuration Editor->System.WebServer/httpCompression/staticTypes"
                }
                else {
                    $output += "`n Value - image/svg+xml - is not found in IIS Configuration Editor->System.WebServer/httpCompression/staticTypes"
                    $ourObject | Add-Member -MemberType NoteProperty -Name "Compression Setting SVGXML StaticType" -Value "Value - image/svg+xml - is not found in IIS Configuration Editor->System.WebServer/httpCompression/staticTypes"
                }
            }
        }
        catch {
